...
namespace CrystalCards.Api.Tests
{
    public class BasicProjectTests: IClassFixture<CustomWebApplicationFactory<Startup>>
    {
        ...
        [Fact]
        public async Task Can_remove_card_from_project()
        {
            // arrange
            ...
            await Client.PostAsync(addCardToProjectRequest.Url,
                ContentHelper.GetStringContent(addCardToProjectRequest.Body));
            //act

            var removeCardFromProjectRequest = new
            {
                Url = $"api/projects/RemoveCardFromProject/{projectId}",
                Body = new
                {
                    projectId = projectId,
                    cardId = cardId
            ...
            var project = await Client.PostAsync(removeCardFromProjectRequest.Url,
                ContentHelper.GetStringContent(removeCardFromProjectRequest.Body));
            ...
            Assert.Equal(expectedCardCount,projectResponse.Cards.Count);
        }
